create or replace procedure p_mx_hbyh(v_nsrsbh     in varchar2,
  ...
  xypj_1        varchar2(30);
  fr_jinru      int;  --20171127 add 禁入法人类型
begin
  select decode(xydj,
                'null',
                '未评级',
...
--违法违章
  ...
  select max(FR_NAME)
   ...
   --20171127 add
	select count(*) 
	into fr_jinru
	from  hsj_share_holder a
	where share_holder_type like '%法人%'
	and regexp_like(share_holder_name,'典当行|贷款|金融|资产')  
	and ENT_NAME = v_nsrmc
	and not exists (select *
                    from hsj_share_holder b
...
*/
    ...
    select count(*),
	       round(sysdate - to_date(nvl(max(substr(alt_date, 1, 10)), '2011-01-01'),
                                 'yyyy-mm-dd'))
    into bgcs_fr,bgrq_fr
	from
	(select distinct ent_name,alt_item,
	                     dbms_lob.substr(alt_be) alt_be , 
						 dbms_lob.substr(alt_af) alt_af,
						 alt_date,
						 lrsj 
    from HSJ_ALTER a
    where ent_name = v_nsrmc
    and alt_date>= to_char(sysdate - 365 * 2,'yyyy-mm-dd')
	and dbms_lob.substr(alt_be) != dbms_lob.substr(alt_af)
  ...
  select count(distinct case_no)
    into XZCF_GS
    from HSJ_LAWSUIT_DETAIL_BGT a
   where SORT_TIME > to_char(sysdate - 365 * 5, 'yyyy-mm-dd')
     and ent_name = v_nsrmc
     and (pname = v_nsrmc or pname = dbrmc_1)
	 and not exists(select *
                    from HSJ_LAWSUIT_DETAIL_BGT b
                    where b.ent_name = a.ent_name
                    and a.lrsj < b.lrsj - 3 / 24 / 60);
  ...
  if fr_jinru <= 0 then
    ...
    term_1 := 1;
    v_out  := v_out || '股东为典当行、贷款、金融、资产类：' || fr_jinru || ';';
  end if;
  ysx_1 := ysx_1 + term_1;
...
exception
  when others then
    v_out        := substr(sqlerrm, 1, 200) || '预授信不通过;';
