...
begin
  ...
  select sum(CURR_OVERDUE_AMOUNT)
   ...
   where REPORT_NO = v_REPORT_NO; --借款人信用卡逾期金额超限

  select sum(CURR_OVERDUE_AMOUNT)
    into OVERDUE_AMOUNT_loan
    from zx_icr_loan_info
   where REPORT_NO = v_REPORT_NO; --借款人贷款逾期金额超限
  ...
  if rzye <= 0 then
    ssxed := greatest(round(ssxed - nvl(BALANCE_ed, 0), -3), 0);
  else
